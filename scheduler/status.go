...
//     2. 处于未初始化状态时，不能变为正在启动或正在停止状态.
//     3. 想要的状态只能是正在初始化、正在启动或正在停止状态中的一个。
...
func checkStatus(
	...
	if currentStatus == SCHED_STATUS_UNINITIALIZED &&
		(wantedStatus == SCHED_STATUS_STARTED||wantedStatus == SCHED_STATUS_STOPPED){
		err = genError("the scheduler has not yet been started!")
		return
	}
	...
	case SCHED_STATUS_INITIALIZING:
		...
		case SCHED_STATUS_STARTED:
			err = genError("the scheduler has heen started!")
		}
	case SCHED_STATUS_STARTING:
		switch currentStatus {
	...
	default:
		err = genError(fmt.Sprintf("unsupported wanted status for check! (wantedStatus: %d)",
			wantedStatus))
